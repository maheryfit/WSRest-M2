...
public class PaiementService {
    ...
    public Optional<Paiement> updatePaiement(Long id, Paiement paiementDetails) {
        return paiementRepository.findById(id).map(paiement -> {
            if (paiementDetails.getStatutPaiement() != null && !paiement.getStatutPaiement().getId().equals(paiementDetails.getStatutPaiement().getId())) {
                HistoriquePaiement historique = new HistoriquePaiement();
                historique.setPaiement(paiement);
                historique.setStatutPaiement(paiementDetails.getStatutPaiement());
                historiquePaiementService.createHistoriquePaiement(historique);
            }
            if (paiementDetails.getCommande() != null) paiement.setCommande(paiementDetails.getCommande());
            if (paiementDetails.getMontant() > 0) paiement.setMontant(paiementDetails.getMontant());
            if (paiementDetails.getStatutPaiement() != null) paiement.setStatutPaiement(paiementDetails.getStatutPaiement());
            if (paiementDetails.getDatePaiement() != null) paiement.setDatePaiement(paiementDetails.getDatePaiement());
    ...
    public Optional<Paiement> updateStatutPaiement(Long id, String libelleStatut) {
        return paiementRepository.findById(id).flatMap(paiement -> 
            statutPaiementService.getStatutPaiementByLibelle(libelleStatut).map(statut -> {
