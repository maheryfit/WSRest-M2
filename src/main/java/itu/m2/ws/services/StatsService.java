...
public class StatsService {
    ...
    public List<Map<String, Object>> getTopRestaurants(String fromStr, String toStr) {
        Timestamp from = (fromStr != null && !fromStr.isEmpty()) ? Timestamp.valueOf(LocalDate.parse(fromStr).atStartOfDay()) : null;
        Timestamp to = (toStr != null && !toStr.isEmpty()) ? Timestamp.valueOf(LocalDate.parse(toStr).atTime(LocalTime.MAX)) : null;
    ...
    public List<Map<String, Object>> getRecommandationsRestaurants(Long clientId) {
        return restaurantRepository.findRecommendationsByClientId(clientId);
    }
}
